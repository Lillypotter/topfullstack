...
module.exports=app=>{
    ...
    router.get('/news/init',async(req,res)=>{
        ...
        //从新闻分类下面获得一些新闻
        const newsTitles=['小兵工厂登陆TGC', '秘！来自李元芳的王者荣耀英雄盛会现场报道', '4月15日正式服“东方来客”版本更新公告', '特别创意奖公布', '4月6日抢先服版本更新公告', '特别创意奖公布', '蒙犽皮肤设计大赛最佳创意奖公布', '首届星梦大赛最终投票结果公示', '互动小任务第22期-五五亲密度道具方案票选结果公布', '互动小任务第22期-五五朋友节专属亲密度道具票选活动开启!']
    ...
    router.get('/news/list',async(req,res)=>{
        ...
        //给每个article加上categoryname，被分到那个分类，categoryName就是哪个
        cats.map(cat=>{
    ...
    router.get('/heroes/list',async(req,res)=>{
        ...
        const cats=await Category.aggregate([
            //聚合管道可以一次执行多个操作
            // 1.找到parentid,相当于where条件查询
            {$match:{parent:parent._id}},
            //找到id下的newsList，相当于from heroes，where heroes.xx=categories.xx
            {$lookup:{
                from:'heroes',
                localField:'_id',
                foreignField:'categories',
